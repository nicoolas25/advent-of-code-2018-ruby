...
guard_log = guard_logs.values.max_by(&:total_minutes_asleep)
total_minutes_asleep = guard_log.total_minutes_asleep
minute, _times = guard_log.most_slept_minute_and_times

puts "Most sleeping guard: #{guard_log.guard_id} with #{total_minutes_asleep} min"
puts "Most slept minute: #{minute}"
puts "Answer is: #{guard_log.guard_id * minute}"

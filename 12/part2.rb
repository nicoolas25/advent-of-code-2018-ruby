...
(1..target).each do |t|
  ...
  if state.to_s == next_state.to_s
    speed = next_state.first_index - state.first_index
    remaining_iterations = target - t
    translation = speed * remaining_iterations
    puts next_state.sum(translation: translation)
    exit
  else
    state = next_state
  end
